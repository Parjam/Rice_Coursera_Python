...
def draw(canvas):
    ...
    # restart the game if there's no lives
    ...
    # call to process_sprite with rock_group
    process_sprite_group(canvas, rock_group)
    process_sprite_group(canvas, missile_group)
    group_group_collide(missile_group, rock_group)
    process_sprite_group(canvas, explosion_group)

    # reduce lives in case of collision
...
# helper function process_sprites
def process_sprite_group(canvas, group):
    for item in group:
        if item.missile_active():
            item.draw(canvas)
            item.update()
        else:
            group.remove(item)
...
# helper function for group to group collide
def group_group_collide(group1, group2):
    global score, explosion_group
    for item in group1:
        copy = group_collide(group2, item)
        if copy:
           position = item.get_position()
           blast = Sprite(position, [0,0], 0, 0, explosion_image, explosion_info, explosion_sound)
